			...
			if ($isFilter): ?>
				<div class="bx-sidebar-block">
					...
					$APPLICATION->IncludeComponent("bitrix:catalog.smart.filter", "bootstrap_v4", array(
							...
							"SECTION_ID" => $arCurSection?$arCurSection['ID']:0, // id "0" if it's not-a-section (main catalog)
							"SHOW_ALL_WO_SECTION" => $arCurSection?"N":"Y", // show all if it's not-a-section (main catalog)
		...
		if ($_GET['sort'])
		{
			switch ($_GET['sort'])
			{
				...
				case 'price_asc':
					...
					break;
				case 'pop_desc':
					$sortField = 'PROPERTY_ORDERS_COUNT';
					$sortOrder = 'desc';
					break;
				case 'pop_asc':
					$sortField = 'PROPERTY_ORDERS_COUNT';
